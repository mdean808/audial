...
<div class='w-8/12 mx-auto mt-2'>
    ...
    <div>
        <ul class='ml-4 mt-2'>
            <li class='text-blue-500'>{officialAttempts.length}/{daysBetweenDates(new Date(), FIRST_DAY)} audials
                attempted.
            </li>
    ...
    <div>
        <ul class='ml-4 mt-2'>
            ...
            <li class='text-gray-300'>{customRandomAttempts.length} <span class='text-red-600'>random</span> <span
                ...
                with {(customRandomAttempts.filter(a => a.correct).length / customRandomAttempts.length * 100) || 0}%
                correct.
            </li>
        </ul>
    </div>
